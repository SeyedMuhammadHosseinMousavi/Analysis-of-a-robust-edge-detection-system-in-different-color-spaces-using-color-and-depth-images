...
% [finr fing finb K]=Finalresyiq(I); 
%
mean=0;
variance=0.01;
noisedensity=0.1;
...
[finr fing finb K]=Finalresyiq(gaussiannoise); Kgauss=K;
figure;
saltynoise = imnoise(I,'salt & pepper',noisedensity);
[finr fing finb K]=Finalresyiq(saltynoise);Ksalt=K;
figure;
poissonnoise = imnoise(I,'poisson');
[finr fing finb K]=Finalresyiq(poissonnoise);Kpoisson=K;
figure;
specklenoise = imnoise(I,'speckle',specklevariance);
[finr fing finb K]=Finalresyiq(specklenoise);Kspeckle=K;
%% morphology
%imerode
se = strel('disk',1);
Kgaussmorph = imerode(Kgauss,se);
Ksaltmorph = imerode(Ksalt,se);
Kpoissonmorph = imerode(Kpoisson,se);subimage(Kpoissonmorph);
Kspecklemorph = imerode(Kspeckle,se);
%imclose
se = strel('disk',1);
Kgaussclose = imclose(Kgaussmorph,se);
Ksaltclose = imclose(Ksaltmorph,se);
Kpoissonclose = imclose(Kpoissonmorph,se);subimage(Kpoissonclose);
