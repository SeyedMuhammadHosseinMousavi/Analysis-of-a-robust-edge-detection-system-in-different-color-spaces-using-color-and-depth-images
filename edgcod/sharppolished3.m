function [sharp3,pic]=sharppolished3(pic)
% clear;clc;
% pre processing
...
for i=1:d1
    ...
    if polished(i,j)<60
        ...
        sharp3(i,j)=polished(i,j);
    end
    end
end
figure;
imshow(sharp3);title('Edges salt & pepper');
figure('units','normalized','outerposition',[0 0 1 1])
imshow(pic2);title('RGB Smooth with medfilt2 and salt & pepper Noise');
figure('units','normalized','outerposition',[0 0 1 1])
imshow(pic3);title('GrayScale Smooth and salt & pepper Noise');
figure('units','normalized','outerposition',[0 0 1 1])
imshow(pic4);title('GrayScale Smooth with wiener2 and salt & pepper Noise');
end