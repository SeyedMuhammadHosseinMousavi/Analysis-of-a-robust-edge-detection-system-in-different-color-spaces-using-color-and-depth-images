function [sharp,pic]=sharppolishednew(pic)
% clear;clc;
% pre processing
...
%
fourier2=fft2(pic);
fouriershift=fftshift(fourier2);
hilp=zeros(512,512);
for i=1:512
    for j=1:512
        hilp(i,j)=1/(1+(((i-256)^2+(j-256)^2)/d0)/n);
    end
end
butterworth=fouriershift.*hilp;
ifs=ifftshift(butterworth);
pic=ifft2(ifs);
pic = real(pic);
pic=uint8(pic);
