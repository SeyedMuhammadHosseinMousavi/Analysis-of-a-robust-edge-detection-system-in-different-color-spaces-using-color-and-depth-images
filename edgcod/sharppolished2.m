function [sharp2,pic]=sharppolished2(pic)
% clear;clc;
% pre processing
...
for i=1:d1
    ...
    if polished(i,j)<60
        ...
        sharp2(i,j)=polished(i,j);
    end
    end
end
figure;
imshow(sharp2);title('Edges gaussian');
figure('units','normalized','outerposition',[0 0 1 1])
imshow(pic2);title('RGB Smooth with medfilt2 and gaussian Noise');
figure('units','normalized','outerposition',[0 0 1 1])
imshow(pic3);title('GrayScale Smooth and gaussian Noise');
figure('units','normalized','outerposition',[0 0 1 1])
imshow(pic4);title('GrayScale Smooth with wiener2 and gaussian Noise');
end
